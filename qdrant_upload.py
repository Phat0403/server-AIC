...
def create_qdrant(collection_name):
    if client_qdrant.get_collection(collection_name=collection_name):
        client_qdrant.delete(collection_name=collection_name)
...
file_clip_feature=os.listdir('C:/Users/nidim2004/Desktop/nidim/server/clip-features')


def upload_data(n):
    for i in range(n):
        points = []
        clip_feature = np.load(f'C:/Users/nidim2004/Desktop/nidim/server/clip-features/' + file_clip_feature[i])
        video = convert_name_file(file_clip_feature[i])
        print(i,file_clip_feature)
        print(i,file_clip_feature[i])
